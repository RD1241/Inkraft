...
**STATUS (2026-06-27 EOD):** DONE — #1 headline (all styles); #2 credits text now "3" everywhere incl. `register.html`; #4 cooler colour-coded style chips; multi-style hero showcase (the switcher was hidden by an uncapped `.hero-comic-img` height — capped to 520px, all 5 style samples now visible/swappable, verified). PENDING — panel-count selector (#3, NOT on the form yet), colour/B&W toggle UI + backend (#4), plus items 5–6 below.
